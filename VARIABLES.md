...
| `RUNNER_RECORDS_DIR` | One durable record per archived session, `~/.cache/<agent>/records/` unless set — when it ran, what it was, what it spent, which commits it made, which runner built its container. Written once when every field in it is final and never rewritten, which is what lets `just records` append one small file per session rather than rewrite a growing one: git stores whole blobs, and a single file rewritten forty times a day would push the whole cache each time. |
