...
| `RUNNER_SAMPLE_SECONDS` | Seconds between host-wide sysstat samples while `run` or `chat` is running, from just before the container starts to the end of the script, so the collection is sampled too. Written by `sadc` under `$RUNNER_CACHE_DIR/sysstat/` as daily `saDD` files, each replaced a month later. Unset, `0` or not a number is off; a host where `sadc` is in none of the places sysstat installs it skips it in silence. |
