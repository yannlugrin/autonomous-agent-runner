...
Runs `host/archive/mirror.sh` and reads its exit status rather than recomputing anything: two implementations
of "is the backup running" is one that drifts. Three answers — 0 ran, 1 stopped, 2 could not be read, which is
`LOOK`: unreachable is never ok here, and it is not a mechanism that stopped either. A `FAIL` with no reason
means the recipe died before it judged, and the probe carries the last line it printed.
See docs/archive.md#a-reading-that-failed-is-not-a-judgement and #the-run-is-not-the-backup.
