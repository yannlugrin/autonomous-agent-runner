...
push. Then run `just setup-archive` in the runner, which clones it.

The mirror is not seeded from here: it has a repository of its own, and
`examples/mirror/` is its seed.
...
**The mirror of the agent's memory is not here.** It lives in a repository of
its own — `examples/mirror/` seeds it — because this one is written by the
machine that runs the agent, and a record that machine can rewrite is not a
record. Nothing here is authoritative either: `sessions` is a copy of what the
volume held at collection time, read-only, and restoring from it is a deliberate
act rather than a sync.
...
| `REPLACE_WITH_PREFIX` | the same, uppercased, dashes as underscores. It prefixes the secret names the runner writes |
...
`just setup-mirror` sets the first two and asks you for the token. The rest
